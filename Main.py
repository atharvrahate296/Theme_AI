...
def landing_page(event=None):
    ...
    button_login.bind("<Button-1>",login_page)
    button_signup.bind("<Button-1>",signup_page)
...
def login_page(event=None):
    ...
    canvas.tag_bind(back, "<Button-1>", signup_page)
...
def signup_page(event=None):
    ...
    canvas.tag_bind(back, "<Button-1>", login_page)
    ...
    def check_email_pass_validity(event=None):
        ...
        if  not email:
            ...
            messagebox.showerror(title="Enter Valid Password", message="Password must contain at least one uppercase letter, one lowercase letter, one numeric digit, and be 8 or more characters long",parent=w3)
        else:
            # All validations passed, proceed to next step 
            global s_email
            s_mail=si_email.get()
            ov.enter_otp_screen(s_mail,sname,pass2)
            w3.after(5000,w3.destroy())
    ...
    send_op.bind("<Button-1>",check_email_pass_validity)
...
def main_screen(name=None,event=None):
    ...
    l=Label(w4,textvariable=greet,font=("Consolas",20,"bold"))
    l.pack()
    ...
    Press 'enter' to continue....
            '''
    l1=Label(w4,text=intro,font=("Montserrat",14,'italic'))
    l1.pack()
    l1.place(relx=0.5,rely=0.5,anchor=CENTER)

    w4.bind("<KeyPress>",m.chat_screen)
